...
export class NPODetailComponent extends PageBaseComponent implements OnInit, AfterViewInit {
    ...
    create() {
        ...
        this._npoService.save<any>(this.id, this.npo).subscribe(resp => {
            // console.log(resp);
            if (resp.is_error == false) {
                this.npo = resp.data;
                super.showsuccess('Non-PO created successful: ' + this.npo.afp_no);
                super.unblockui('#m_form_1');
                // this.navigate_list();
            } else {
                super.showError(resp.error_msg);
    ...
    update() {
        ...
        this._npoService.save<any>(this.id, this.npo).subscribe(resp => {
            if (resp.is_error == false) {
                this.npo = resp.data;
                super.showsuccess('Non-PO updated successful: ' + this.npo.afp_no);
                super.unblockui('#m_form_1');
                this.navigate_list();
            } else {
                super.showError(resp.error_msg);
