...
export class PMService extends TokenBaseService {
    ...
    public get<T>(id: string) {
        ...
        return this.http.get(API_PAYMENT_GET_PUT_DEL + '/' + id, super.jwt())
            .map((response: Response) => <T>response.json());
    }

    public create<T>(pm: PM) {

        return this.http.post(API_PAYMENT_INSERT, pm, super.jwt())
            .map((response: Response) => <T>response.json());
    }

    public put<T>(pm: PM) {

        return this.http.put(API_PAYMENT_GET_PUT_DEL + '/' + pm.payment_id, pm, super.jwt())
            .map((response: Response) => <T>response.json());
    }
